...
少なくとも「moved な値を borrow するのはエラーで、ここでは三行目で move された `x` を四行目で利用しようとしたがそれはできない」までは容易に読み取れると思います。なので対策としては三行目で move しないようにする…！と進んでいけるわけです。
...
Rust では `let` で変数を定義します^[JavaScript の `let` を連想すると思いますが、これはどちらかというと（まじでどちらかと言うならば） `const` に近い意味合いです。しかしシャドーイングは許可されているが（再代入だけではなく）値の変更は許可されておらず、そういった点で JavaScript の気持ちが適用できない感は強いです。とりあえず `let` で怒られたら `mut mut` を使うみたいな運用でいいと思います。]。ここでは `client` という名前をつけています。
